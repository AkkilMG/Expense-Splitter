...
import { Router } from 'express';
import { authMiddleWare } from '../middleware';
import { validateExpense, insertExpense, getExpenseById, updateExpenseSettledMembers, revertExpenseSettledMember } from '../models';
import { getGroupById } from '../models';
import { getUserNamesByIds } from '../models';
import { calculateSplit } from '../services';
...
router.post('/', authMiddleWare, async (req, res) => {
  ...
  const members = await getUserNamesByIds(group.members);
...
router.get('/group/:groupId/member/:memberId', authMiddleWare, async (req, res) => {
  const { groupId, memberId } = req.params;
  const query = 'SELECT * FROM expenses WHERE group_id = ?';
  const expenses = await client.execute(query, [groupId], { prepare: true });

  const activeExpenses = expenses.rows.filter(expense => {
    return expense.settled_members.indexOf(memberId) === -1 && !expense.is_settled;
  });

  const settledExpenses = expenses.rows.filter(expense => {
    return expense.settled_members.indexOf(memberId) > -1 || expense.is_settled;
  });
...
router.post('/:expenseId/settle/:memberId', async (req, res) => {
  ...
  await updateExpenseSettledMembers(expense, memberId);
  res.send(expense);
});
...
router.post('/:expenseId/revert/:memberId', async (req, res) => {
  ...
  await revertExpenseSettledMember(expense, memberId);
  res.send(expense);
});
