...
import _ from "lodash";
import { authMiddleWare } from "../middleware";
import { validate, insertUser, getUserByEmail, getUserById, getUsersByIds } from "../models";
import { getExpensesByPaidBy, getOwedExpensesByUserId } from "../models";
import { UserType } from "../types";
...
router.post("/", async (req, res) => {
  ...
  var user: any = await getUserByEmail(req.body.email);
  if (user) return res.status(400).send("User already registered.");
  ...
  const token = user.generateAuthToken(); // Make sure you have this function implemented in your user model
  res
    .header("Authorization", token)
    .header("access-control-expose-headers", "Authorization")
  ...
  const lent = paidByExpenses.reduce((previousValue, currentValue) => {
    ...
    const settledExpenses =
      currentValue.settledMembers.length > 0
        ? currentValue.settledMembers.map((member) => {
            return currentValue.membersBalance.find(
              (memberBalance) => memberBalance.memberId === member
            )?.balance;
          })
        : [];
...
router.get("/", async (req, res) => {
  ...
  const result = await getUsersByIds(parsedUserIds);
  return res.send(result);
});
